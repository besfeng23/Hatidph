...
type TrustedContact = { id: string; name: string; phone: string; alert: string };
type SupportTicket = { id: string; issue: string; status: 'Open' | 'In review' | 'Resolved'; summary: string; createdAt: string };
type ShareStatus = 'idle' | 'generated' | 'copied' | 'sent';
type MapPicker = 'pickup' | 'dropoff' | null;
type RoutePreset = { id: string; label: string; detail: string; eta: string; distance: string; fare: string; traffic: string; route: string };

const pickupPresets: RoutePreset[] = [
  { id: 'lobby', label: 'Salcedo lobby entrance', detail: 'Main lobby on Valero side', eta: '3 min', distance: '7.2 km', fare: '₱212', traffic: 'Moderate', route: 'Ayala Ave → McKinley' },
  { id: 'guardhouse', label: 'Village guardhouse', detail: 'Guardhouse pickup with plate check', eta: '4 min', distance: '7.4 km', fare: '₱218', traffic: 'Moderate', route: 'Valero → Ayala Ave' },
  { id: 'basement', label: 'Basement pickup bay', detail: 'Basement B1 waiting area', eta: '5 min', distance: '7.5 km', fare: '₱220', traffic: 'Busy', route: 'Dela Rosa exit → Ayala Ave' },
  { id: 'corner', label: 'Valero corner pickup', detail: 'Corner pickup for faster approach', eta: '2 min', distance: '7.1 km', fare: '₱209', traffic: 'Light', route: 'Valero → Paseo' },
];

const dropoffPresets: RoutePreset[] = [
  { id: 'bgc', label: 'BGC High Street', detail: '5th Ave drop-off bay', eta: '18 min', distance: '7.2 km', fare: '₱212', traffic: 'Moderate', route: 'Ayala Ave → McKinley' },
  { id: 'naia3', label: 'NAIA Terminal 3', detail: 'Departures curbside lane', eta: '31 min', distance: '9.8 km', fare: '₱318', traffic: 'Busy', route: 'EDSA → Skyway access' },
  { id: 'ayala', label: 'Ayala Center Makati', detail: 'Glorietta pickup/drop-off bay', eta: '9 min', distance: '2.1 km', fare: '₱128', traffic: 'Light', route: 'Paseo → Ayala Center' },
  { id: 'moa', label: 'SM Mall of Asia', detail: 'Main transport bay', eta: '28 min', distance: '8.7 km', fare: '₱288', traffic: 'Busy', route: 'Buendia → Macapagal' },
];
...
const stateCopy: Record<TripStage, { title: string; detail: string; cta: string }> = {
  ...
  arriving: { title: 'Driver approaching pickup', detail: `${driver.name} is ${driver.distance}. Meet only at your selected pickup point.`, cta: 'Driver arrived' },
  ...
  in_trip: { title: 'Trip in progress', detail: 'Live trip is shared with Mom. ETA updates as the route preset changes.', cta: 'Arriving soon' },
  arriving_soon: { title: 'Arriving soon', detail: 'Prepare to exit safely at the selected drop-off bay.', cta: 'Complete trip' },
...
const cancelCopy: Partial<Record<TripStage, { title: string; message: string; confirm: string }>> = {
  ...
  arriving: { title: 'Cancel while driver is approaching?', message: `${driver.name} is already on the way to your selected pickup point. Keep the ride open if you still plan to board.`, confirm: 'Cancel ride' },
...
export default function App() {
  ...
  const [pickupPoint, setPickupPoint] = useState<RoutePreset>(pickupPresets[0]);
  const [dropoffPoint, setDropoffPoint] = useState<RoutePreset>(dropoffPresets[0]);
  const [mapPicker, setMapPicker] = useState<MapPicker>(null);
  ...
  const [contactRemoveId, setContactRemoveId] = useState<string | null>(null);
  const [toast, setToast] = useState<string | null>(null);
  const [rating, setRating] = useState(0);
  const [feedbackTags, setFeedbackTags] = useState<string[]>([]);
  const [trustedContacts, setTrustedContacts] = useState<TrustedContact[]>([
    { id: 'mom', name: 'Mom', phone: '+63 917 000 4100', alert: 'Live trip sharing enabled' },
    { id: 'brother', name: 'Brother', phone: '+63 918 555 2200', alert: 'SOS alerts enabled' },
  ]);
  ...
  const contactPhoneValid = /^9\d{9}$/.test(normalizePhone(contactPhone));
  const contactFormValid = Boolean(contactName.trim()) && contactPhoneValid;
  ...
  const contactToRemove = trustedContacts.find((contact) => contact.id === contactRemoveId) ?? null;
  ...
  function updatePickup(point: RoutePreset) {
    setPickupPoint(point);
    setMapPicker(null);
    notify(`Pickup set: ${point.label}`);
  }

  function updateDropoff(point: RoutePreset) {
    setDropoffPoint(point);
    setMapPicker(null);
    notify(`Drop-off set: ${point.label}`);
  }
  ...
  function startEditContact(contact: TrustedContact) {
    setEditingContactId(contact.id);
    setContactName(contact.name);
    setContactPhone(contact.phone);
    setContactAlert(contact.alert);
    notify(`Editing ${contact.name}`);
  }

  function saveTrustedContact() {
    if (!contactFormValid) {
      notify('Enter a valid PH mobile number');
      return;
    }
    const formattedPhone = `+63 ${formatPhone(normalizePhone(contactPhone))}`;
    if (editingContactId) {
      setTrustedContacts((contacts) => contacts.map((contact) => contact.id === editingContactId ? { ...contact, name: contactName.trim(), phone: formattedPhone, alert: contactAlert } : contact));
      notify('Trusted contact updated');
      clearContactForm();
      return;
    }
    setTrustedContacts((contacts) => [...contacts, { id: `contact-${Date.now()}`, name: contactName.trim(), phone: formattedPhone, alert: contactAlert }]);
    notify('Trusted contact added');
    clearContactForm();
  }

  function confirmRemoveContact() {
    if (!contactRemoveId) return;
    setTrustedContacts((contacts) => contacts.filter((contact) => contact.id !== contactRemoveId));
    setContactRemoveId(null);
    notify('Trusted contact removed');
  }
  ...
  function createTicket() {
    const ticket: SupportTicket = {
      ...
      issue: selectedIssue,
      status: 'Open',
      summary: issueDetails.trim() || 'No extra details provided yet.',
      createdAt: 'Just now',
    };
    setTicketCreated(true);
    setSupportTicket(ticket);
    notify(`Support ticket ${ticket.id} created`);
    open('Support ticket created', `Reference ${ticket.id} created for ${selectedIssue}. Trip context, route, driver, plate, and receipt were attached in the preview.`);
  }
  ...
  return (
    <PhoneShell>
      ...
      {screen === 'home' && <Home go={go} open={open} notify={notify} pickupPoint={pickupPoint} dropoffPoint={dropoffPoint} openPicker={setMapPicker} />}
      {screen === 'search' && <Search go={go} back={back} setDropoff={setDropoffPoint} notify={notify} />}
      {screen === 'choose' && <ChooseRide back={back} selectedRide={selectedRide} setSelectedRide={setSelectedRide} pickupInstruction={pickupInstruction} setPickupInstruction={setPickupInstruction} paymentMethod={paymentMethod} setPaymentMethod={setPaymentMethod} beginTrip={beginTrip} pickupPoint={pickupPoint} dropoffPoint={dropoffPoint} openPicker={setMapPicker} />}
      {screen === 'activeTrip' && <ActiveTrip go={go} open={open} tripStage={tripStage} selectedRide={selectedRide} pickupInstruction={pickupInstruction} paymentMethod={paymentMethod} nextTripStep={nextTripStep} cancelTrip={cancelTrip} pickupPoint={pickupPoint} dropoffPoint={dropoffPoint} />}
      ...
      {screen === 'receipt' && <Receipt go={go} back={back} open={open} selectedRide={selectedRide} paymentMethod={paymentMethod} rebookTrip={rebookSameTrip} returnHome={returnHomeAfterTrip} pickupPoint={pickupPoint} dropoffPoint={dropoffPoint} />}
      {screen === 'trips' && <Trips go={go} />}
      {screen === 'balance' && <Balance open={open} />}
      {screen === 'safety' && <Safety go={go} open={open} trustedContacts={trustedContacts} supportTicket={supportTicket} />}
      {screen === 'account' && <Account go={go} open={open} />}
      {screen === 'trustedContacts' && <TrustedContacts back={back} contacts={trustedContacts} editingContactId={editingContactId} name={contactName} phone={contactPhone} alert={contactAlert} contactValid={contactFormValid} setName={setContactName} setPhone={setContactPhone} setAlert={setContactAlert} saveContact={saveTrustedContact} clearForm={clearContactForm} startEdit={startEditContact} promptRemove={setContactRemoveId} />}
      {screen === 'shareTrip' && <ShareTrip back={back} contacts={trustedContacts} status={shareStatus} setStatus={setShareStatus} open={open} notify={notify} pickupPoint={pickupPoint} dropoffPoint={dropoffPoint} />}
      {screen === 'reportIssue' && <ReportIssue back={back} selectedIssue={selectedIssue} setSelectedIssue={setSelectedIssue} details={issueDetails} setDetails={setIssueDetails} supportTicket={supportTicket} createTicket={createTicket} go={go} />}
      ...
      {mapPicker && <MapPickerSheet kind={mapPicker} pickupPoint={pickupPoint} dropoffPoint={dropoffPoint} choosePickup={updatePickup} chooseDropoff={updateDropoff} close={() => setMapPicker(null)} />}
      {cancelPrompt && <CancelRideModal stage={cancelPrompt} keepRide={() => setCancelPrompt(null)} confirmCancel={finalizeCancelTrip} />}
      {contactToRemove && <RemoveContactModal contact={contactToRemove} keep={() => setContactRemoveId(null)} remove={confirmRemoveContact} />}
...
function Home({ go, open, notify, pickupPoint, dropoffPoint, openPicker }: { go: (screen: Screen) => void; open: (title: string, message: string) => void; notify: (message: string) => void; pickupPoint: RoutePreset; dropoffPoint: RoutePreset; openPicker: (kind: MapPicker) => void }) {
  return <section className="screen app-screen active"><Topbar title="Good evening, Maria" subtitle={`Pickup: ${pickupPoint.label}`} action={() => notify('No new notifications')} /><div className="content scroll"><div className="live-card"><span>42 drivers nearby</span><b>{pickupPoint.eta} average pickup</b></div><div className="mini-map" onClick={() => go('choose')}><RouteMap stage="idle" compact /><div><b>Pickup accuracy: High</b><span>{pickupPoint.detail} • {dropoffPoint.eta} to {dropoffPoint.label}</span></div></div><div className="split-actions"><button className="secondary" onClick={() => openPicker('pickup')}>Adjust pickup</button><button className="secondary" onClick={() => openPicker('dropoff')}>Change drop-off</button></div><h1>Saan ang punta?</h1><button className="search-box" onClick={() => go('search')}><span className="inline-glyph-row"><Glyph code="Q" /> {dropoffPoint.label}</span><em>{dropoffPoint.eta}</em></button><div className="status-grid premium-grid">{serviceStatus.map(([name, status, count]) => <div key={name}><b>{name}</b><span>{status}</span><small>{count}</small></div>)}</div><div className="promo"><b>Route refreshed</b><span>{pickupPoint.label} → {dropoffPoint.label} • {dropoffPoint.route}</span></div><button className="balance-card" onClick={() => go('balance')}><span>Hatid Balance</span><b>₱1,250.00</b><small>Ride credits linked to GCash, Maya, Cash</small></button><h3>Saved places</h3><div className="quick-grid"><button onClick={() => go('choose')}><Glyph code="H" /> Home</button><button onClick={() => go('choose')}><Glyph code="W" /> Work</button><button onClick={() => open('Saved places', 'Saved place creation is ready as a placeholder flow.')}><Glyph code="+" /> Add place</button></div><h3>Recent trip</h3><button className="list-row" onClick={() => go('receipt')}><b>{pickupPoint.label} → {dropoffPoint.label}</b><small>Completed • {dropoffPoint.fare} • Rebook available</small></button></div></section>;
}

function Search({ go, back, setDropoff, notify }: { go: (screen: Screen) => void; back: () => void; setDropoff: (value: RoutePreset) => void; notify: (message: string) => void }) {
  return <section className="screen app-screen active"><Topbar title="Set destination" back={back} /><div className="content scroll"><div className="route-inputs"><input value={`Current Location • ${places.pickup}`} readOnly /><input placeholder="Where to?" autoFocus /></div><h3>Popular destinations</h3>{dropoffPresets.map((place) => <button key={place.id} className="list-row" onClick={() => { setDropoff(place); notify(`Drop-off set: ${place.label}`); go('choose'); }}><b>{place.label}</b><small>{place.detail} • {place.eta} • {place.fare}</small></button>)}<h3>More places</h3>{popularDestinations.map(([name, detail]) => <button key={name} className="list-row" onClick={() => go('choose')}><b>{name}</b><small>{detail}</small></button>)}</div></section>;
}

function ChooseRide({ back, selectedRide, setSelectedRide, pickupInstruction, setPickupInstruction, paymentMethod, setPaymentMethod, beginTrip, pickupPoint, dropoffPoint, openPicker }: { back: () => void; selectedRide: (typeof rideOptions)[number]; setSelectedRide: (ride: (typeof rideOptions)[number]) => void; pickupInstruction: string; setPickupInstruction: (value: string) => void; paymentMethod: (typeof paymentMethods)[number]; setPaymentMethod: (value: (typeof paymentMethods)[number]) => void; beginTrip: () => void; pickupPoint: RoutePreset; dropoffPoint: RoutePreset; openPicker: (kind: MapPicker) => void }) {
  return <section className="screen app-screen map-screen active"><div className="map-panel"><button className="map-back" onClick={back}>←</button><RouteMap stage="idle" /></div><div className="sheet booking-sheet"><div className="handle" /><p className="eyebrow">{pickupPoint.label} → {dropoffPoint.label} • {dropoffPoint.distance} • ETA {dropoffPoint.eta}</p><div className="quote-hero"><div><span>Estimated total</span><b>{dropoffPoint.fare}</b><small>{selectedRide.bestFor}</small></div><div><span>Traffic</span><b>{dropoffPoint.traffic}</b><small>{dropoffPoint.route}</small></div></div><div className="fare-intel"><div><b>Pickup</b><span>{pickupPoint.eta} • {pickupPoint.detail}</span></div><div><b>Dropoff</b><span>{dropoffPoint.eta} • {dropoffPoint.detail}</span></div><div><b>Policy</b><span>{selectedRide.cancellation}</span></div></div><div className="split-actions"><button className="secondary" onClick={() => openPicker('pickup')}>Adjust pickup</button><button className="secondary" onClick={() => openPicker('dropoff')}>Change drop-off</button></div>{rideOptions.map((ride) => <button key={ride.id} className={selectedRide.id === ride.id ? 'ride-option-v2 selected' : 'ride-option-v2'} onClick={() => setSelectedRide(ride)}><RideGlyph kind={ride.icon} /><span className="ride-copy"><b>{ride.label}</b><small>{ride.eta} • {ride.trip}</small><em>{ride.routeNote}</em></span><span className="ride-price"><b>{ride.id === selectedRide.id ? dropoffPoint.fare : ride.fare}</b><small>{ride.available}</small></span></button>)}<section className="booking-card"><div className="section-head"><b>Pickup instruction</b><span>{pickupPoint.detail}</span></div><div className="choice-row">{pickupInstructions.map((item) => <button key={item} className={pickupInstruction === item ? 'choice active' : 'choice'} onClick={() => setPickupInstruction(item)}>{item}</button>)}</div></section><section className="booking-card"><div className="section-head"><b>Payment</b><span>{paymentMethod.label}</span></div><div className="payment-list">{paymentMethods.map((method) => <button key={method.id} className={paymentMethod.id === method.id ? 'payment-option active' : 'payment-option'} onClick={() => setPaymentMethod(method)}><b>{method.label}</b><span>{method.detail}</span><em>{method.status}</em></button>)}</div></section><section className="booking-card fare-card"><div className="section-head"><b>Fare breakdown</b><span>Mock route recalculation</span></div>{fareBreakdown.map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}<div><span>Selected route estimate</span><b>{dropoffPoint.fare}</b></div><div className="promo-line"><span>{promo.label}</span><b>-{promo.discount}</b></div></section><div className="policy-note"><b>{promo.code}</b><span>{promo.detail}. {selectedRide.cancellation}.</span></div><button className="primary" onClick={beginTrip}>Confirm {selectedRide.label.replace('Hatid ', '')}</button></div></section>;
}

function ActiveTrip({ go, open, tripStage, selectedRide, pickupInstruction, paymentMethod, nextTripStep, cancelTrip, pickupPoint, dropoffPoint }: { go: (screen: Screen) => void; open: (title: string, message: string) => void; tripStage: TripStage; selectedRide: (typeof rideOptions)[number]; pickupInstruction: string; paymentMethod: (typeof paymentMethods)[number]; nextTripStep: () => void; cancelTrip: () => void; pickupPoint: RoutePreset; dropoffPoint: RoutePreset }) {
  const copy = stateCopy[tripStage];
  return <section className="screen app-screen map-screen active"><div className="status-card elevated"><b>{copy.title}</b><span>{tripStageLabels[tripStage]}</span><small>{copy.detail}</small></div><div className="map-panel full"><RouteMap stage={tripStage} /></div><div className="sheet compact emotional"><div className="progress-rail">{tripTimeline.map((stage) => <span key={stage} className={stageIndex(stage) <= stageIndex(tripStage) ? 'done' : ''} />)}</div><div className="trip-stage"><b>{copy.title}</b><span>{copy.detail}</span></div><DriverCard compact /><div className="pin"><span>Your Ride PIN</span><b>4821</b></div><div className="trip-facts"><div><b>Pickup</b><span>{pickupPoint.label} • {pickupInstruction}</span></div><div><b>Drop-off</b><span>{dropoffPoint.label}</span></div><div><b>Payment</b><span>{paymentMethod.label}</span></div><div><b>ETA</b><span>{dropoffPoint.eta}</span></div></div><div className="actions"><button onClick={() => go('shareTrip')}>Share Trip</button><button onClick={() => open('Message driver', 'Production will open masked in-app chat/call with safety logging.')}>Message</button><button className="danger" onClick={() => open('Emergency SOS confirmation', 'Demo only — no emergency call is placed. Production would ask for confirmation, notify trusted contacts, share live location, and attach trip context to Hatid Safety.')}>Emergency</button></div><div className="split-actions"><button className="secondary" onClick={cancelTrip}>Cancel ride</button><button className="primary" onClick={nextTripStep}>{copy.cta}</button></div><button className="link center" onClick={() => go('driverVerification')}>View driver verification</button><small className="microcopy">{selectedRide.label} • {dropoffPoint.fare} • Trip PIN required before boarding.</small></div></section>;
}
...
function Receipt({ go, back, open, selectedRide, paymentMethod, rebookTrip, returnHome, pickupPoint, dropoffPoint }: { go: (screen: Screen) => void; back: () => void; open: (title: string, message: string) => void; selectedRide: (typeof rideOptions)[number]; paymentMethod: (typeof paymentMethods)[number]; rebookTrip: () => void; returnHome: () => void; pickupPoint: RoutePreset; dropoffPoint: RoutePreset }) {
  return <section className="screen app-screen active"><Topbar title="Trip receipt" back={back} /><div className="content scroll"><div className="receipt elevated"><p>Total paid</p><h2>{dropoffPoint.fare}.00</h2><small>June 11, 2026 • HTD-98213-AB29</small><hr />{fareBreakdown.map(([label, value]) => <div key={label}><span>{label}</span><b>{value}</b></div>)}<div><span>Selected route estimate</span><b>{dropoffPoint.fare}</b></div><hr /><div><span>Ride type</span><b>{selectedRide.label}</b></div><div><span>Driver</span><b>{driver.name}</b></div><div><span>Route</span><b>{pickupPoint.label} → {dropoffPoint.label}</b></div><div><span>Payment</span><b>{paymentMethod.label}</b></div></div><div className="receipt-actions"><button className="secondary" onClick={() => open('Download receipt', 'Production will create a PDF receipt and email copy.')}>Download / Share receipt</button><button className="primary" onClick={rebookTrip}>Rebook same trip</button><button className="secondary" onClick={() => go('trips')}>Back to trips</button><button className="secondary" onClick={returnHome}>Return Home</button><button className="link center danger-link" onClick={() => go('reportIssue')}>Report issue</button></div></div></section>;
}
...
function Safety({ go, open, trustedContacts, supportTicket }: { go: (screen: Screen) => void; open: (title: string, message: string) => void; trustedContacts: TrustedContact[]; supportTicket: SupportTicket | null }) {
  return <section className="screen app-screen active"><Topbar title="Safety Center" /><div className="content scroll"><div className="sos-card"><h2>Emergency SOS</h2><p>Demo only — no emergency call will be placed. Production requires confirmation, live location sharing, and trusted contact notification.</p><button className="danger" onClick={() => open('Emergency SOS confirmation', `Demo mode only. In production this would call emergency support, notify ${trustedContacts.map((c) => c.name).join(' and ')}, share live location, and attach trip HTD-98213-AB29.`)}>Open SOS confirmation</button></div>{supportTicket && <button className="list-row selected" onClick={() => go('reportIssue')}><b>Support ticket status</b><small>{supportTicket.id} • {supportTicket.status} • {supportTicket.issue}</small></button>}<button className="list-row" onClick={() => go('trustedContacts')}><b>Trusted contacts</b><small>{trustedContacts.length} contacts ready for live trip links and SOS alerts</small></button><button className="list-row" onClick={() => go('shareTrip')}><b>Share live trip preview</b><small>Generate link, copy, or send to family</small></button><button className="list-row" onClick={() => go('reportIssue')}><b>Report issue</b><small>Create a support ticket with trip context</small></button><button className="list-row" onClick={() => go('driverVerification')}><b>Driver verification</b><small>Plate match, PIN, identity, vehicle profile</small></button><button className="list-row" onClick={() => go('safetyGuide')}><b>Passenger safety guidelines</b><small>Before boarding, during trip, after arrival</small></button></div></section>;
}

function Account({ go, open }: { go: (screen: Screen) => void; open: (title: string, message: string) => void }) {
  return <section className="screen app-screen active"><Topbar title="Account" /><div className="content scroll"><div className="profile-card"><div className="driver-photo-v2">MS</div><div><h2>Maria Santos</h2><p>+63 968 184 1001</p><span>KYC Verified</span></div></div><button className="list-row" onClick={() => open('Edit profile', 'Edit profile flow placeholder.')}>Edit profile</button><button className="list-row" onClick={() => go('trustedContacts')}>Trusted contacts</button><button className="list-row" onClick={() => go('terms')}>Terms & Privacy</button><button className="list-row" onClick={() => open('Delete account', 'Production will require identity confirmation and data retention notice.')}>Delete account</button><button className="list-row" onClick={() => open('App version', 'Hatid preview v0.8.1 phase 8D safety flow depth.')}>App version</button></div></section>;
}

function TrustedContacts({ back, contacts, editingContactId, name, phone, alert, contactValid, setName, setPhone, setAlert, saveContact, clearForm, startEdit, promptRemove }: { back: () => void; contacts: TrustedContact[]; editingContactId: string | null; name: string; phone: string; alert: string; contactValid: boolean; setName: (value: string) => void; setPhone: (value: string) => void; setAlert: (value: string) => void; saveContact: () => void; clearForm: () => void; startEdit: (contact: TrustedContact) => void; promptRemove: (id: string) => void }) {
  const phoneHasValue = phone.trim().length > 0;
  return <section className="screen app-screen active"><Topbar title="Trusted contacts" back={back} /><div className="content scroll"><p className="note">Contacts receive trip links only when you choose to share or trigger SOS.</p>{contacts.map((contact) => <div className="contact-card" key={contact.id}><b>{contact.name}</b><span>{contact.phone}</span><em>{contact.alert}</em><div className="split-actions"><button className="secondary" onClick={() => startEdit(contact)}>Edit</button><button className="secondary" onClick={() => promptRemove(contact.id)}>Remove</button></div></div>)}<section className="booking-card"><div className="section-head"><b>{editingContactId ? 'Edit trusted contact' : 'Add trusted contact'}</b><span>PH mobile only</span></div><label className="field">Name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Ate, Mom, Brother" /></label><label className="field">Phone<input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="+63 9XX XXX XXXX" /></label><p className={phoneHasValue && !contactValid ? 'hint error' : 'hint'}>{phoneHasValue && !contactValid ? 'Enter a valid PH mobile number starting with 9.' : 'Use a reachable Philippine mobile number.'}</p><div className="choice-row">{alertTypes.map((item) => <button key={item} className={alert === item ? 'choice active' : 'choice'} onClick={() => setAlert(item)}>{item}</button>)}</div><button className="primary" disabled={!contactValid} onClick={saveContact}>{editingContactId ? 'Save contact' : 'Add trusted contact'}</button>{editingContactId && <button className="secondary" onClick={clearForm}>Cancel edit</button>}</section></div></section>;
}

function ShareTrip({ back, contacts, status, setStatus, open, notify, pickupPoint, dropoffPoint }: { back: () => void; contacts: TrustedContact[]; status: ShareStatus; setStatus: (value: ShareStatus) => void; open: (title: string, message: string) => void; notify: (message: string) => void; pickupPoint: RoutePreset; dropoffPoint: RoutePreset }) {
  const link = 'hatid.ph/t/HTD-98213-AB29';
  return <section className="screen app-screen active"><Topbar title="Share live trip" back={back} /><div className="content scroll share-trip-content"><RouteMap stage="arriving" compact /><div className="share-card"><b>Live trip preview</b><span>Maria is riding with {driver.name} in {driver.color} {driver.vehicle} {driver.plate}.</span><small>Pickup: {pickupPoint.label} • Dropoff: {dropoffPoint.label} • ETA {dropoffPoint.eta}</small></div>{status !== 'idle' && <div className="note"><b>{status === 'generated' ? 'Secure link generated' : status === 'copied' ? 'Secure link copied' : 'Trip sent to trusted contacts'}</b><br />{link}</div>}<button className="primary" onClick={() => { setStatus('generated'); notify('Secure trip link generated'); }}>Generate secure trip link</button><button className="secondary" onClick={() => { setStatus('copied'); notify('Trip link copied'); }}>Copy secure link</button><button className="secondary" onClick={() => { setStatus('sent'); open('Trip shared', `Sent to ${contacts.map((c) => c.name).join(', ')}.`); }}>Send to trusted contacts</button><h3>Recipients</h3>{contacts.map((contact) => <div className="contact-card" key={contact.id}><b>{contact.name}</b><span>{contact.phone}</span><em>{contact.alert}</em></div>)}</div></section>;
}

function ReportIssue({ back, selectedIssue, setSelectedIssue, details, setDetails, supportTicket, createTicket, go }: { back: () => void; selectedIssue: string; setSelectedIssue: (value: string) => void; details: string; setDetails: (value: string) => void; supportTicket: SupportTicket | null; createTicket: () => void; go: (screen: Screen) => void }) {
  return <section className="screen app-screen active"><Topbar title="Report issue" back={back} /><div className="content scroll"><p className="note">Trip context, driver, route, plate, and receipt can be attached to the support ticket.</p>{supportTicket && <div className="sos-card"><h2>Ticket status: {supportTicket.status}</h2><p>{supportTicket.id} • {supportTicket.issue} • Created {supportTicket.createdAt}</p><p>{supportTicket.summary}</p><button className="secondary" onClick={() => go('safety')}>Back to Safety Center</button></div>}<h3>Issue type</h3>{issueTypes.map((item) => <button key={item} className={selectedIssue === item ? 'list-row selected' : 'list-row'} onClick={() => setSelectedIssue(item)}><b>{item}</b><small>Attach trip HTD-98213-AB29</small></button>)}<label className="field">What happened?<textarea value={details} onChange={(event) => setDetails(event.target.value)} placeholder="Describe what happened. Include location, time, and safety concern if needed." rows={4} /></label><button className="primary" onClick={createTicket}>{supportTicket ? 'Create another support ticket' : 'Create support ticket'}</button></div></section>;
}
...
function MapPickerSheet({ kind, pickupPoint, dropoffPoint, choosePickup, chooseDropoff, close }: { kind: Exclude<MapPicker, null>; pickupPoint: RoutePreset; dropoffPoint: RoutePreset; choosePickup: (point: RoutePreset) => void; chooseDropoff: (point: RoutePreset) => void; close: () => void }) {
  const options = kind === 'pickup' ? pickupPresets : dropoffPresets;
  const selected = kind === 'pickup' ? pickupPoint.id : dropoffPoint.id;
  return <div className="modal" role="dialog" aria-modal="true"><div className="modal-card elevated"><h2>{kind === 'pickup' ? 'Adjust pickup' : 'Change drop-off'}</h2><p>{kind === 'pickup' ? 'Choose the safest pickup point for this demo route.' : 'Choose a preset destination to refresh ETA, route, and fare copy.'}</p>{options.map((option) => <button key={option.id} className={selected === option.id ? 'list-row selected' : 'list-row'} onClick={() => kind === 'pickup' ? choosePickup(option) : chooseDropoff(option)}><b>{option.label}</b><small>{option.detail} • {option.eta} • {option.fare}</small></button>)}<button className="secondary" onClick={close}>Keep current</button></div></div>;
}
...
function RemoveContactModal({ contact, keep, remove }: { contact: TrustedContact; keep: () => void; remove: () => void }) {
  return <div className="modal" role="dialog" aria-modal="true"><div className="modal-card elevated"><h2>Remove trusted contact?</h2><p>{contact.name} will stop appearing as a trip sharing recipient in this preview.</p><div className="modal-actions"><button className="primary" onClick={keep}>Keep contact</button><button className="secondary danger" onClick={remove}>Remove contact</button></div></div></div>;
}
